...
$$('[data-run]').forEach(button=>button.onclick=async()=>{
 ...
 try{
   ...
   worker.postMessage({type:'run',id,code:lab.querySelector('textarea').value,plot});const result=await promise;
 ...
 }catch(e){status.textContent=e.message;}finally{button.disabled=false;lab.querySelector('[data-cancel]').hidden=true;}
});

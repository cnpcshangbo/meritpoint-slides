...
(() => {
  ...
  const keyPrefix = 'mpa-python-trial-lab:v1:';
  ...
  function renderResult(section, result, restored = false) {
    ...
    field(section,'[data-status]').textContent = restored ? 'Saved text result · run again to verify and redraw charts.' : result.ok ? 'Ran successfully. What changed?' : 'Read the last error line. What could you try?';
  }
  ...
  function exportLab() {
    ...
    download('my-python-trial-lab.json',JSON.stringify({format:'meritpoint-python-trial-lab',version:1,savedAt:new Date().toISOString(),stages:exported},null,2));
  }
  for (const section of sections) {
    section.addEventListener('input', event => {
      ...
      if (target.matches('.code')) {
        if (task?.section === section) stop('Run stopped because the code changed.');
    ...
    field(section,'[data-pad]').onclick = async()=>{
      ...
      const id = await window.PythonWorkspace.loadSnippet({code,name:'Lab · '+field(section,'h2').textContent,packages:packages(code),origin:{deck:'python/trial-lab',slide:section.id}});
  ...
  $('[data-download]').onclick = ()=>download('lab-'+sections[index].id+'.py',field(sections[index],'.code').value,'text/x-python');
  ...
  $('[data-import-file]').onchange = async event=>{
    ...
    try {
      ...
      if (data.format !== 'meritpoint-python-trial-lab' || data.version !== 1 || !data.stages || sections.some(s=>typeof data.stages[s.id]?.code !== 'string' || data.stages[s.id].code.length>20000)) throw new Error('This is not a complete Python Trial Lab backup.');
